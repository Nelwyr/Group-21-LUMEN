...
`story-audiences.js` renders all four segments from `strategy_evidence.json`, including full-precision monthly beverage spend and separate price-threshold sample sizes. The two editorial priority segments are shared in `recommendation.js`. Students have the largest sample but their EUR2.2036 mean threshold is slightly above EUR2.19, not below it; a group mean cannot establish individual reachability.
...
The original city screen markup now lives under `index.html#city`, and the original launch-window markup under `index.html#timing`. `city-ui.js`, `city-engine.js` and `launch-window.js` are unchanged; the story invokes their existing initialisers. The simulator no longer contains or initialises either screen. Berlin and April decision boxes use the prepared inputs and existing `rankCities` / `assessLaunch` functions; changing weights or month explores alternatives without changing the stated recommendation. Existing city and launch browser suites now exercise the relocated screens, including reliability, diagnostics, presets, failures and retry.
